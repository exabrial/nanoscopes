package com.github.exabrial.cdi.nanoscoped.messagescoped.testing;
...
import com.github.exabrial.cdi.nanoscoped.messagescoped.MessageScoped;
...
public class MessageScopedLifecycleController {
	...
	public void activate(final String unitName, final String displayName) {
		...
		log.trace("activate() MessageScope:{}", scopeKey);
	...
	public void deactivate(final String unitName, final String displayName) {
		...
		log.trace("deactivate() MessageScope:{}", scopeKey);
	...
	static Bean<?> getBean(final BeanManager bm) {
		...
		if (bean == null) {
			synchronized (MessageScopedLifecycleController.class) {
				bean = CACHED_BEAN;
				if (bean == null) {
					final Set<Bean<?>> beans = bm.getBeans(MessageScopedLifecycleController.class);
	...
	static MessageScopedLifecycleController obtain(final BeanManager bm) {
		...
		return (MessageScopedLifecycleController) bm.getReference(bean, MessageScopedLifecycleController.class, ctx);
	}
}
